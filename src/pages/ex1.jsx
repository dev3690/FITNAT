...
import { getData, PATIENT, USER, callAxiosApi, insertPatient, updateData, deleteData, birdViewApi } from 'src/utils/api_utils';
...
const handleReset = () => {
  if (currentPatient?.id) {
    ...
    toast.warning("Please save the patient information first");
  }
};

// Update the reset confirmation handler
const handleResetConfirmation = async (confirmed) => {
  if (confirmed && currentPatient?.id) {
    try {
      const response = await callAxiosApi(birdViewApi);
      const birdViewData = response.data?.data || response.data;
      
      const patientBirdView = birdViewData?.find(item => 
        item.patient_id === currentPatient.id
      );

      if (patientBirdView) {
        const resetData = {
          table: 'statusUpdate',
          id: patientBirdView.id,
        ...
        await callAxiosApi(updateData, resetData);
        setisDataUpdated(!isDataUpdated);
        toast.success("Week 1 status reset successfully");
      } else {
        toast.warning("No data found for this patient");
      }
    } catch (error) {
      console.error('Failed to reset week status:', error);
      toast.error("Failed to reset week status");
    }
  }
